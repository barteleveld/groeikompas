        ...
        <details className="group overflow-hidden rounded-3xl border border-rose-100 bg-white shadow-sm shadow-rose-950/5">
          <summary data-testid="modules-summary" className="flex cursor-pointer list-none flex-col gap-5 p-5 marker:content-none sm:p-6 lg:flex-row lg:items-center lg:justify-between [&::-webkit-details-marker]:hidden">
            <div className="flex min-w-0 items-start gap-4">
              ...
              <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wider text-teal-700">Totaaloverzicht</p><h3 className="mt-1 text-xl font-black">{visibleModules.length} modules Â· {assignments.length} opdrachten</h3><p className="mt-1 text-sm text-slate-600">{completedAssignments} afgerond, {activeAssignments} bezig en {assignments.length - completedAssignments - activeAssignments} nog te starten</p></div>
            </div>
                        ...
                        return (
                          <details id={assignmentAnchor(assignment)} className="group/assignment scroll-mt-6 overflow-hidden rounded-2xl border border-slate-100 bg-slate-50" key={assignment}>
                            <summary data-testid={`assignment-summary-${assignmentAnchor(assignment)}`} className="flex cursor-pointer list-none flex-wrap items-start justify-between gap-3 p-4 [&::-webkit-details-marker]:hidden">
                              <div className="flex min-w-0 items-start gap-3"><span className={`grid size-8 shrink-0 place-items-center rounded-full text-xs font-black ${status === "Afgerond" ? "bg-emerald-100 text-emerald-800" : "bg-white text-slate-500 shadow-sm"}`}>{status === "Afgerond" ? <CheckCircle2 className="size-4" aria-hidden /> : index + 1}</span><div><p className="font-bold">{assignment}</p><p className="mt-1 text-xs text-slate-500">{goals.length} leerdoelen Â· {submissions.length} versies Â· {feedbackHistory.length} feedbackrondes</p>{latestFeedback && !latestFeedback.response && <p className="mt-1 text-xs font-semibold text-fuchsia-800">Feedback vraagt om een reactie</p>}</div></div>
                            ...
                            <div className="space-y-5 border-t border-slate-200 bg-white p-4 sm:p-5">
                              ...
                              <section>
                                ...
                                {submissions.length ? <ol className="mt-3 space-y-2">{submissions.map((submission, submissionIndex) => <li className="rounded-xl bg-slate-50 p-3 text-sm" key={`${submission}-${submissionIndex}`}><strong>Versie {submissionIndex + 1}</strong> Â· {submission}</li>)}</ol> : <p className="mt-2 text-sm text-slate-500">Nog niets ingeleverd.</p>}
