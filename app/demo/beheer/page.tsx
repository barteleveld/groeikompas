...
import { demoClasses, useDemo } from "@/components/demo/demo-state";

type Tab = "goals" | "users" | "modules";

export default function AdminDemo() {
  const { state, addUser, addLearningGoal, updateLearningGoal, toggleLearningGoalArchive, toggleUserActive, updateUserCohort } = useDemo();
  const [tab, setTab] = useState<Tab>("goals");
  const [message, setMessage] = useState("");
  const [goalMessage, setGoalMessage] = useState("");
  const cards = [
    { n: state.users.filter((user) => user.role === "Student" && user.active).length + 8, label: "studenten", icon: GraduationCap },
    { n: state.users.filter((user) => user.role === "Docent" && user.active).length + 1, label: "docenten", icon: School },
    { n: new Set(state.modules.flatMap((module) => module.assignments)).size - state.archivedAssignments.length, label: "opdrachten", icon: BookOpenCheck },
    { n: state.goalCatalog.filter((goal) => !goal.archived).length, label: "leerdoelen", icon: Target },
  ];
  ...
  return (
    <>
      ...
      {tab === "goals" && <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <section><h2 className="mb-1 text-xl font-black">Bestaande leerdoelen</h2><p className="mb-4 text-sm text-slate-600">Aanpassingen worden ook doorgevoerd bij opdrachten waaraan dit doel al gekoppeld is.</p><div className="space-y-3">{state.goalCatalog.map((goal) => <details className={`group card overflow-hidden ${goal.archived ? "opacity-70" : ""}`} key={goal.id}><summary className="flex min-h-11 cursor-pointer list-none items-start justify-between gap-3 p-4 [&::-webkit-details-marker]:hidden"><div><span className="text-xs font-bold uppercase tracking-wide text-teal-700">{goal.domain}</span><div className="mt-1 flex flex-wrap items-center gap-2"><p className="font-black">{goal.title}</p>{goal.archived && <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">Gearchiveerd</span>}</div><p className="mt-1 text-sm text-slate-600">{goal.description}</p></div><Pencil className="mt-1 size-4 shrink-0 text-teal-700" aria-label="Aanpassen" /></summary><div className="space-y-4 border-t border-slate-200 p-4">{!goal.archived && <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); updateLearningGoal(goal.id, goalFromForm(new FormData(event.currentTarget))); setGoalMessage(`${goal.title} is bijgewerkt.`); }}><label className="block text-sm font-bold">Titel<input className="field mt-1" name="title" defaultValue={goal.title} required /></label><label className="block text-sm font-bold">Domein<input className="field mt-1" name="domain" defaultValue={goal.domain} required /></label><label className="block text-sm font-bold">Beschrijving<textarea className="field mt-1" name="description" defaultValue={goal.description} required /></label><button className="min-h-11 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-bold text-white">Wijzigingen opslaan</button></form>}<button type="button" onClick={() => toggleLearningGoalArchive(goal.id)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold">{goal.archived ? "Leerdoel herstellen" : "Leerdoel archiveren"}</button></div></details>)}</div></section>
      </div>}
      ...
      {tab === "users" && <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <section><h2 className="mb-4 text-xl font-black">Gebruikers</h2><div className="space-y-3">{state.users.map((user) => <details className={`card p-4 ${user.active ? "" : "bg-slate-50 opacity-70"}`} key={user.id}><summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3"><div><p className="font-black">{user.name}</p><p className="mt-1 text-xs text-slate-500">{user.cohort}</p></div><span className={`rounded-full px-2 py-1 text-xs font-bold ${user.active ? "bg-teal-50 text-teal-900" : "bg-slate-200 text-slate-700"}`}>{user.active ? user.role : "Gedeactiveerd"}</span></summary><div className="mt-4 space-y-3 border-t border-slate-200 pt-4"><label className="block text-sm font-bold">Klas<select className="field mt-1" value={user.cohort} onChange={(event) => updateUserCohort(user.id, event.target.value)}>{[...demoClasses].map((className) => <option key={className}>{className}</option>)}</select></label><div className="flex flex-col gap-2 sm:flex-row"><button type="button" onClick={() => setMessage(`Uitnodiging voor ${user.name} is opnieuw verstuurd.`)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold">Uitnodiging opnieuw sturen</button><button type="button" onClick={() => toggleUserActive(user.id)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold">{user.active ? "Gebruiker deactiveren" : "Gebruiker activeren"}</button></div></div></details>)}</div></section>
      </div>}
