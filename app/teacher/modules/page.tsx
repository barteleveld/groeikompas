...
export default async function ModulesPage() {
  ...
  const [{data:modules},{data:cohortRows},{data:assignments},{data:cohortLinks},{data:assignmentLinks}] = await Promise.all([
    ...
    profile.role === "admin" ? supabase.from("cohorts").select("id,name").is("archived_at", null) : supabase.from("teacher_cohorts").select("cohort:cohorts(id,name,archived_at)").eq("teacher_id", profile.id),
    supabase.from("assignments").select("id,title").is("archived_at", null).order("created_at"),
  ...
  const cohorts=(cohortRows??[]).map((r:any)=>r.cohort?(Array.isArray(r.cohort)?r.cohort[0]:r.cohort):r).filter((cohort:any)=>cohort&&!cohort.archived_at);
