...
export default async function ModulesPage() {
  ...
  return <><PageHeader eyebrow="Leerlijn" title="Modules" description="Bundel opdrachten in een herkenbare route. Studenten zien wat eerst komt en waar de module naartoe werkt."/><div className="grid gap-7 xl:grid-cols-[.8fr_1.2fr]"><section className="card h-fit p-5"><h2 className="mb-4 text-xl font-black">Nieuwe module</h2><ModuleForm cohorts={cohorts} assignments={assignments??[]}/></section><section><h2 className="mb-4 text-xl font-black">Bestaande modules</h2><div className="space-y-4">{(modules??[]).map((m:any)=>{const item={...m,cohortIds:(cohortLinks??[]).filter((x:any)=>x.module_id===m.id).map((x:any)=>x.cohort_id),assignmentIds:(assignmentLinks??[]).filter((x:any)=>x.module_id===m.id).sort((a:any,b:any)=>a.sort_order-b.sort_order).map((x:any)=>x.assignment_id)};return <details className="card p-5" key={m.id}><summary className="cursor-pointer list-none"><div className="flex justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-teal-700">{m.period||"Doorlopend"}</p><h3 className="mt-1 text-lg font-black">{m.title}</h3><p className="mt-1 text-sm text-slate-600">{item.assignmentIds.length} opdrachten</p></div><span className={`h-fit rounded-full px-2 py-1 text-xs font-bold ${m.is_published?"bg-emerald-50 text-emerald-900":"bg-slate-100"}`}>{m.is_published?"Zichtbaar":"Concept"}</span></div></summary><div className="mt-5 border-t border-slate-200 pt-5"><ModuleForm module={item} cohorts={cohorts} assignments={assignments??[]}/></div></details>})}</div></section></div></>;
}
